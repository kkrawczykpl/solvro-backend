import { plainToClass } from 'class-transformer';
import { validate, ValidationError } from 'class-validator';
import { RequestHandler } from 'express';

/**
 * Transforms a plain js object to instance of specific class
 * After that validates the object and if there are errors
 * returns them.
 * @param type - any
 * @param skipMissingProperties - boolean, false default
 */
function validationMiddleware(type: any, skipMissingProperties = false): RequestHandler {
  return (req, res, next) => {
    validate(plainToClass(type, req.body), { skipMissingProperties })
    .then((errors: ValidationError[]) => {
      if (errors.length > 0) {
        const message = errors.map((error: ValidationError) => Object.values(error.constraints!));
        res.status(400).json(JSON.stringify(message));
      } else {
        next();
      }
    });
  };
}

export default validationMiddleware;